"""Raise the serial Sina history request rate.
...
CONFIG_ID = "10000000-0000-0000-0000-000000000029"
    ...
    op.execute(
        ...
        inserted AS (
            ...
            SELECT '{CONFIG_ID}', latest.version + 1, 'SINA',
                   'use validated 500ms serial history interval'
            FROM latest
            RETURNING version
        )
        ...
        SELECT gen_random_uuid(), inserted.version, 'SINA', current.capability,
               ...
               CASE WHEN current.capability LIKE 'HISTORICAL_DAILY_%'
                    THEN 1 ELSE current.concurrency END,
               CASE WHEN current.capability LIKE 'HISTORICAL_DAILY_%'
                    THEN 2.0 ELSE current.rate_per_second END,
