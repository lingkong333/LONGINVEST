...
def create_async_client(
    *, connect_timeout: float = 3, read_timeout: float = 5,
    write_timeout: float = 3, pool_timeout: float = 1,
) -> httpx.AsyncClient:
...
class ProviderHttpClient:
    ...
    def _validate_headers(self, response: httpx.Response) -> None:
        if sum(len(k) + len(v) for k, v in response.headers.items()) > self._max_header_bytes:
            raise ProviderHttpError("PROVIDER_RESPONSE_TOO_LARGE")
    ...
    def _decode(self, body: bytes) -> dict[str, Any]:
        lowered = body.lower()
        if any(marker in lowered for marker in (b"<html", b"captcha", b"verify", b"validatecode")):
            code = "PROVIDER_CAPTCHA_DETECTED" if b"<html" not in lowered else "PROVIDER_UNEXPECTED_CONTENT"
