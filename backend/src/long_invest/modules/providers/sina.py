...
class SinaRealtimeProvider:
    ...
    async def realtime_quotes(
        ...
        text = await self._client.request_text(
            ProviderHttpRequest(self.REALTIME_URL + codes), deadline=deadline, encoding="gb18030"
        )
        return self.parse_quotes(text, symbols, received_at=datetime.now(timezone.utc))
    ...
    async def probe(self, capability: ProviderCapability, deadline: datetime) -> ProbeResult:
        ...
        if capability is not ProviderCapability.REALTIME_QUOTE_BATCH:
            return ProbeResult(self.code, capability, False, datetime.now(timezone.utc), 0, "PROVIDER_CAPABILITY_UNSUPPORTED")
        try:
    ...
    def parse_quotes(
        ...
        if len(text.encode("utf-8")) > 256_000 or any(marker in text.lower() for marker in ("<html", "captcha", "validatecode", "error")):
        ...
        for raw_line in filter(None, (line.strip() for line in text.splitlines())):
            ...
            if len(fields) < 32:
                raise ProviderHttpError("PROVIDER_SCHEMA_INCOMPATIBLE")
            try:
                quote_time = datetime.fromisoformat(f"{fields[30]}T{fields[31]}").replace(tzinfo=CHINA_TIMEZONE)
                values = [Decimal(fields[index]) for index in (1, 2, 3, 4, 5, 8, 9)]
                parsed[symbol] = RealtimeQuote(
                    symbol, values[2], values[0], values[3], values[4], values[1],
                    int(values[5]), values[6], quote_time, received_at, self.code,
                )
