...
class FakeService:
    ...
    async def claim_forecast(self, _task_id, _training):
        self.calls.append("claim_forecast")

    async def freeze_forecast(self, _task_id, training, result, *, frozen_at):
    ...
    async def claim_simulation(self, _task_id, _test_data):
        self.calls.append("claim_simulation")

    async def save_success(self, _task_id, _test_data, _result):
        self.calls.append("save_success")
        return "done"

    async def fail(self, _task_id, code):
...
async def _run_holdout_workflow() -> None:
    ...
    result = await application.run(task.id)
    ...
    assert engine.kwargs["bars"][0].trade_date == date(2025, 12, 31)
