...
| 数据源 | 东方财富负责实时与当日日线；新浪/AKShare 负责完整历史日线 |
| 核心日线时点 | A股交易日 17:00 |
...
- 当日日线若东方财富失败不自动切换；完整历史优先新浪，东方财富历史只作为人工诊断后备。
...
| 数据库 | PostgreSQL |
| 缓存/队列 | Redis、RQ |
| HTTP | HTTPX；东方财富历史日线使用服务器 Playwright Chromium 传输 |
| 重试 | Tenacity |
| 密码 | Argon2id |
| 策略数据 | pandas、NumPy |
...
- 每个进程复用 HTTP Client 和连接池；东方财富历史日线使用 V3.8 规定的
  服务器 Chromium 传输，其余请求继续使用 HTTPX。
...
|---|---:|---:|---:|---:|
| 东方财富 | 主源 | 主源 | 当日日线主源、历史非自动后备 | 历史非自动后备 |
| 新浪/AKShare | 备用 | 备用 | 完整历史主源 | 完整历史主源 |
...
全市场双口径历史回填是独立任务，支持单股、选择股票、监控列表和全市场时间范围。任务启动冻结股票范围和日期，父任务下每股一个 `job_item`，每股依次获取未复权与前复权、独立校验并由两个数据模块分别提交。
...
任务逻辑并发默认 4，可配置 1～8；新浪历史能力最多四并发，所有并发共享至少 500 毫秒的全局请求间隔。使用 `bulk-history` 队列，与实时、日线和回测 Worker 隔离。全市场回测优先复用回填生成的前复权数据集。

```mermaid
flowchart TD
    A["冻结股票与日期范围"] --> B["父 job + 每股 job_item"]
    B --> C["bulk-history Worker 领取一只股票"]
    C --> D["通过 AKShare 低频获取新浪双口径历史"]
    D --> E["分别校验代码、日期、OHLC、重复、锚点和覆盖率"]
    E --> F{"校验结果"}
...
- 每股未复权 UPSERT 幂等，同日存在不同值走数据修订流程；前复权内容相同复用当前版本，不同内容原子切换新版本。
...
磁盘 95% 暂停回填；Provider 熔断时项目等待或失败，不占死 Worker；单股数据错误不影响其他股票；服务器重启从数据库状态恢复；原始响应不能无限积累。任务详情必须显示股票级进度、当前阶段、双口径写入计数、错误码和截断说明。
