...
- 单机默认部署允许多个隔离进程由同一后台容器监督，但权限不同或依赖显著不同的
  角色必须分容器运行。
...
开发和当前部署统一使用 Linux + Docker Compose。PostgreSQL、Redis、API、前端、
普通后台、策略与回测后台、历史采集后台使用项目独立的容器、网络和数据卷；后台
容器内部仍按角色运行独立进程。PostgreSQL 与 Redis 不直接暴露到公网。API 开发
端口只绑定回环地址，正式公网入口、TLS、备份和高可用另行设计。
...
默认单机部署使用监督进程在普通后台和策略后台容器中启动各角色；实时、通知渠道、
策略和批量任务仍使用独立进程，保持队列、超时和失败隔离。历史采集保持专用容器。
...
任务逻辑并发默认 4，可配置任意正整数；新浪历史能力按任务快照中的页面填写值实际并发，所有并发共享至少 500 毫秒的全局请求间隔。使用 `bulk-history` 队列，与实时、日线和回测 Worker 隔离。全市场回测优先复用回填生成的前复权数据集。
