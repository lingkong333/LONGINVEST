...
V3.1 完整继承用户提供的 V3.0 原文，仅修正已经确认的项目实施约束：

1. 明确开发和当前部署环境为 Linux + Docker Compose；正式生产部署仍不在本版范围内。
2. 基础阶段拆成公共底座与横切能力两个验收门槛；日志、审计和通知公共能力先于业务模块。
3. 补全逻辑任务状态图中的 `WAITING_RETRY`、`BLOCKED` 和 `REJECTED`，使图与正式状态枚举一致。
4. 公共底座验收后，独立子模块每批最多并行三个；共享入口、迁移链和公共契约由主流程串行维护。
...
| 回测 | 逐日防未来函数时序、D/D+1 成交线、父任务与每股项目并发 |
...
任务创建时冻结：股票范围、日期、初始资金、策略源码和哈希、策略参数、环境版本、滞回配置、回测规则版本和数据来源。
...
1. 额外获取策略要求的预热数据。
2. 在交易日 D 只向策略提供 D 及之前的数据。
3. 计算 D 日四档目标。
4. 使用 D 日收盘价和正式信号规则判断区间。
5. 生成待执行订单。
6. 在 D+1 的下一个有效开盘价成交。
...
四级验证：语法/契约、固定样本、用户指定股票试算、完整单股回测。发布验证绑定精确源码哈希；代码、元数据、参数 Schema 变化后失效。完整回测可以没有交易或盈利，但不能有未处理执行和数据错误。
...
回测任务冻结模式、范围、日期、初始资金、策略源码和哈希、参数、API 版本、运行环境、滞回、规则版本、数据源和复权口径。草稿任务复制源码，不引用持续变化的当前草稿。

#### 25.14.2 数据和预热

统一使用东方财富前复权日线。监控股可复用已验证数据，全市场按股票临时获取。保存获取时间、范围、行数和数据哈希，但不承诺未来第三方历史完全复现。

根据策略 `min_bars/max_bars` 额外向回测开始日前获取预热历史。预热只计算目标，不产生交易。历史不足项目 `INSUFFICIENT_HISTORY`。

#### 25.14.3 防未来函数

对每个模拟日 D：策略只看到不晚于 D 的数据；计算 D 目标；D 收盘价进入与生产相同的区间和滞回逻辑；收盘后产生订单；D+1 下一个有效开盘执行。策略代码只返回目标序列，交易和收益由可信回测引擎计算。

策略沙箱不能一次把未来完整数据暴露给用户函数。实现可由可信 Runner 按日期逐步传入切片；沙箱仍受 CPU、内存、超时和网络限制。
...
父状态 `PENDING/RUNNING/PAUSING/PAUSED/SUCCEEDED/PARTIAL/FAILED/CANCELING/CANCELED`。子项状态细分 `PENDING/FETCHING_DATA/VALIDATING_DATA/RUNNING_STRATEGY/SIMULATING/SAVING/SUCCEEDED/FAILED/SKIPPED/CANCELED`。

SINGLE 保存每日目标、状态和权益；WATCHLIST/MARKET 只保存每股指标和交易，不保存所有每日曲线。
...
策略发布所需单股回测必须绑定当前源码哈希、满足最小历史、完整完成且无未处理错误。不要求有交易或盈利。

前复权无法取得、关键数据缺失、OHLC 无效、任何日期策略超时、目标无效、未来数据泄漏、历史不足或结果保存失败使该股票失败。合理非交易日、已知停牌、无交易、期末持仓和末日未成交不使任务失败。
