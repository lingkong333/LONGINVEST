...
以下 V3.15 及更早修订说明为不可修改的实施历史，不是 V4.0 当前施工要求。凡涉及
Redis、RQ、独立 Worker、浏览器采集、旧任务拆分或旧部署数量的描述，均以 V4.0
修订说明和本版正文为准。
...
- 组合回测、组合资金分配和组合净值。
- 分钟线、逐笔行情的长期存储。
- Tushare 等付费数据服务依赖；AKShare 仅作为已选公开数据源的 SDK 适配层使用。
...
notifications        统一通知、模板、邮件、企业微信、飞书和投递历史
...
普通后台只有一个常驻进程，通过数据库优先级和执行槽隔离任务：`P0` 为实时和紧急通知，`P1` 为当日日线，`P2` 为普通业务，`P3` 为历史回填和全市场回测。低优先级任务不得占满全部 Provider 请求预算、数据库连接或执行槽。
...
- 完成训练期目标、测试期模拟、候选筛选和人工加入监视中心。
- 建立统一通知入口并完成邮件、企业微信和飞书适配。
...
V4 新任务不写 Redis 发件箱。需要事务后异步执行时，业务事实、`job` 和审计在同一
...
低优先级任务不得占用为 P0/P1 保留的数据源额度、数据库连接或执行槽。CPU 密集或
...
内部指标包含 HTTP 请求量和耗时、数据库连接池等待、普通后台心跳、各优先级任务积压、任务成功率和超时、Provider 请求额度/成功率/熔断、行情批次缺失、目标失败、通知失败、磁盘、时钟和日历覆盖。指标标签不得使用任意错误全文或高基数用户输入。指标接口不面向公网。
...
`MarketDataProvider` 提供 `capabilities`、股票主数据、批量实时行情、全市场当日日线、单股历史日线和能力探测。第一版实现东方财富与新浪/AKShare 适配器，上层只依赖 `ProviderRouter`。
...
DAILY_BAR_UNADJUSTED: EASTMONEY → SINA（能力启用时）
HISTORICAL_DAILY_UNADJUSTED: SINA_AKSHARE → EASTMONEY_DIAGNOSTIC
HISTORICAL_DAILY_QFQ: SINA_AKSHARE → EASTMONEY_DIAGNOSTIC
```

每项能力分别配置主源、备用源、自动切换开关和可选人工固定来源。主源整体失败、熔断、限流或额度不足时切换备用源；部分缺失时只请求缺失范围。人工固定来源不可用时明确失败，不静默改源。

每条正式数据保存实际来源和请求批次。实时字段不能跨来源拼接；历史数据允许备用源补齐缺失日期，但必须保留逐行来源、先通过统一口径校验，并在来源值冲突时进入质量复核，不能静默覆盖。
...
正式数据、任务进度和后续处理请求统一提交 PostgreSQL；数据库故障时批次不能只存在普通后台内存。磁盘达到 95% 时暂停全市场历史回填和批量任务，实时行情继续。
...
普通后台内置的 Cron 调度每 10 秒使用数据库时间扫描。它只创建 `schedule_occurrence` 和业务任务，不在扫描事务中执行行情、日线或计算。数据库唯一约束和领取租约防止重启或未来多实例产生重复任务。
...
非交易日改交易日：未来时间正常执行，已过去不补跑并告警。交易日改非交易日：待领取任务取消，运行中的行情可保留诊断但不能提交新信号。历史已发生事件不删除，可加日历修正注释。
...
比较应用、数据库、普通后台和行情时间。与数据库差 >5 秒 WARNING，>30 秒 ERROR 并暂停新正式自动调度；已运行任务继续。恢复时间后自动解除，错过批次仍 MISSED。
...
事件：`trading_calendar.updated/coverage_low/missing`、`scheduler.occurrence_created/missed`、`scheduler.clock_skew_detected/recovered`、`daily_market_data.requested`、`quote_cycle.requested`。

普通后台停止时 occurrence 和任务留在 PostgreSQL 待领取；数据库故障时不得只在内存创建调度结果。日历导入一项错误则整个版本不生效并返回逐项校验。
...
全市场双口径历史回填是一个完整业务任务，支持单股、选择股票、监控列表和全市场时间范围。任务启动冻结股票范围和日期，不为每只股票创建通用 `job` 或 `job_item`。任务使用压缩范围快照、游标、分段计数和业务失败清单记录进度；每股依次获取未复权与前复权、独立校验并由两个数据模块分别提交。
...
任务逻辑并发默认 4，可配置任意正整数；实际并发同时受普通后台执行槽、机器资源和 Provider 能力配置约束。新浪历史能力默认共享至少 500 毫秒的最小请求间隔，并与同一 Provider 的其他调用共同扣减持久请求预算。历史回填使用 P3 优先级，不得占用实时和当日日线预留额度。全市场回测优先复用回填生成的前复权数据集。

```mermaid
flowchart TD
    A["冻结股票与日期范围"] --> B["创建一个 job、游标和失败清单"]
    B --> C["普通后台在任务内部领取下一范围"]
