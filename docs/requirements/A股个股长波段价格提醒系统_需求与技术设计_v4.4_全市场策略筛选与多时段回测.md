...
## V4.4 全市场策略筛选与多时段回测增量

V4.4 完整继承 V4.3，并以本节覆盖第 25.10、25.14 和 25.20.9 节的冲突内容：

1. 策略模块负责全市场逐股筛选。一个用户可见筛选任务冻结策略版本、参数、股票范围、多个训练/测试时段和日线数据版本；内部逐股执行，同一股票在每个时段独立产生 `MATCHED + 四档价格` 或 `NOT_MATCHED + 原因`，不得建立数千个顶层任务。
2. 筛选成功形成不可变候选批次。回测模块不提供代码编辑或任意源码入口，只能选择一个候选批次，并使用其精确策略版本、参数、股票和初始四档价格。
3. 每个时段满足 `训练开始 <= 训练结束 < 测试开始 <= 测试结束`。多个时段允许重叠，但后一时段的四个边界日期分别不得早于前一时段对应边界；拖动排序或编辑不得绕过校验。
4. 每只股票、每个时段使用相同初始资金独立回测，不共享资金，不计算组合收益。批次只汇总股票分布、完成情况、收益、回撤、胜率和交易次数。
5. 回测允许用户为单只股票指定交易日创建新的四档价格版本。新版本只影响该日及之后，四档必须同时提交且严格递增；系统从最早受影响日期确定性重算后续结果。
6. 价格回滚不修改历史，而是复制选定历史四档创建新的价格版本，并记录生效日期、原因、操作者和来源版本。公司行动产生的口径调整与用户价格版本分别记录。
7. 单股详情显示日线或 K 线、四档价格、买卖点、价格调整点和回滚点。支撑位、压力位是可拖动的页面观察线；鼠标移入显示日期、OHLC、当日有效四档、支撑位和压力位，观察线不参与交易计算。
8. 每股每时段至少输出买卖次数、完整交易次数、盈亏次数和金额、净收益、总/年化收益率、胜率、盈亏比、最大回撤、单笔极值、持有天数、资金使用率、订单、成交、持仓和每日权益。

验收至少覆盖：全市场逐股失败隔离、候选批次不可变、回测拒绝任意源码、多时段合法重叠与日期倒退拒绝、跨时段独立预测、价格版本按日生效、回滚创建新版本、受影响区间重算、单股曲线与完整指标，以及相同冻结输入的确定性重放。

## V4.3 策略筛选输出增量

V4.3 完整继承 V4.2，并补充以下可执行约束：

1. 数据模块只向策略执行提供冻结的前复权日线快照；策略不得查询数据库、调用数据源或访问网络。
2. 固定入口仍为 `calculate_targets(history, params, context)`。策略先判断股票是否满足自身条件，并返回明确的 `matched` 布尔值。
3. `matched=true` 时必须同时返回 `low_strong`、`low_watch`、`high_watch`、`high_strong` 四档价格；任一缺失、无效或顺序错误均为策略结果错误。
4. `matched=false` 时必须返回非空 `reason`，不得返回四档价格。该结果是正常业务结果，不计为策略执行失败。
5. 目标价计算收到不匹配结果时记录为 `NOT_MATCHED`，不创建候选目标、不覆盖现有目标。回测收到不匹配结果时把该股票记为 `NOT_MATCHED`，不进入测试期交易模拟，其他股票继续执行。
6. 参数 Schema 的每个公开参数必须包含中文 `title` 和非空 `description`；有默认值或数值范围时页面必须直接展示。工作台必须说明标准日线输入、参数作用、匹配结果和四档价格输出，不得只展示原始 JSON Schema。

验收至少覆盖：匹配并返回四档价格、不匹配且有原因、不匹配却返回价格、匹配但缺少价格、目标价不匹配不覆盖、回测单股不匹配隔离，以及参数用途、必填、默认值和范围的中文展示。
...
`history` 是按交易日期升序的前复权 pandas DataFrame，固定包含 `trade_date/open/high/low/close/volume/amount`。策略只能获得目标日期及之前的数据。`params` 只含符合参数 Schema 的冻结配置值；`context` 只含股票身份、目标日期、策略版本、数据版本和计算原因。
...
`matched=false` 是正常筛选结果，不是执行错误；此时禁止返回四档价格。`matched=true` 时禁止省略四档价格。
