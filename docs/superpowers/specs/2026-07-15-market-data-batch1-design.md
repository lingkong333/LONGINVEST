...
2. 补齐 Provider 的路由采集、指定来源采集和日线采集公开端口，使行情 Worker 可以先请求主源，再仅对缺失或无效股票请求备用源，而不访问 Provider 内部对象。
3. 补齐股票模块按显式代码冻结不可变范围的公开端口，手工行情批次不能自行读取股票表构造范围。
4. 准备模块公开端口和测试夹具，不建立跨模块内部依赖。
5. 定义主流程接入点，但暂不让子模块修改路由、迁移主链和生成类型。
