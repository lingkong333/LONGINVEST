  ...
  return (
    <main className="mx-auto flex w-full max-w-[1500px] flex-col gap-6 px-4 py-6 lg:px-8">
      <header className="grid gap-5 border-b pb-5 xl:grid-cols-[1fr_auto] xl:items-end">
        ...
        <nav className="flex max-w-full gap-1 overflow-x-auto rounded-lg border bg-muted/40 p-1" aria-label="通知中心视图">
          {views.map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              type="button"
              className="flex shrink-0 items-center gap-2 rounded-md px-3 py-2 text-sm font-medium aria-current:bg-background aria-current:shadow-sm"
      ...
      {view === "events" ? <EventsView gateway={gateway} onUnauthorized={invalidate} /> : null}
      {view === "deliveries" ? <DeliveriesView gateway={gateway} onUnauthorized={invalidate} /> : null}
      {view === "channels" ? <ChannelsView gateway={gateway} onUnauthorized={invalidate} /> : null}
      {view === "policies" ? <PoliciesView gateway={gateway} onUnauthorized={invalidate} /> : null}
      {view === "templates" ? <TemplatesView gateway={gateway} onUnauthorized={invalidate} /> : null}
    </main>
  )
}
...
function EventsView({
  ...
  return (
    <section className="overflow-hidden rounded-lg border bg-card" aria-label="通知事件列表">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[960px] text-left text-sm">
          <thead className="border-b bg-muted/40 text-muted-foreground">
            <tr><th className="px-4 py-3 font-medium">发生时间</th><th className="px-4 py-3 font-medium">事件</th><th className="px-4 py-3 font-medium">业务对象</th><th className="px-4 py-3 font-medium">资格</th><th className="px-4 py-3 font-medium">渠道</th><th className="px-4 py-3 font-medium">状态</th></tr>
          </thead>
          <tbody className="divide-y">
            {query.data.items.map((event) => (
              <tr key={event.id}>
                <td className="px-4 py-4">{formatTime(event.createdAt)}</td>
                ...
                <td className="px-4 py-4"><span className="block">{event.businessObjectType}</span><span className="mt-1 block max-w-56 truncate font-mono text-xs text-muted-foreground">{event.businessObjectId}</span></td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4">{event.effectiveChannels.length > 0 ? event.effectiveChannels.map((channel) => channelLabels[channel]).join("、") : "仅网页"}</td>
      ...
      <footer className="border-t px-4 py-3 text-sm text-muted-foreground">共 {query.data.total} 个事件</footer>
    </section>
  )
}
...
function DeliveriesView({
  ...
  return (
    <>
      <section className="grid min-h-[34rem] overflow-hidden rounded-lg border bg-card xl:grid-cols-[minmax(0,1fr)_25rem]" aria-label="渠道投递与尝试">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[780px] text-left text-sm">
            <thead className="border-b bg-muted/40 text-muted-foreground">
              <tr><th className="px-4 py-3 font-medium">渠道</th><th className="px-4 py-3 font-medium">状态</th><th className="px-4 py-3 font-medium">代数</th><th className="px-4 py-3 font-medium">尝试</th><th className="px-4 py-3 font-medium">更新时间</th><th className="px-4 py-3 font-medium text-right">操作</th></tr>
            </thead>
            <tbody className="divide-y">
              {query.data.items.map((item) => (
                <tr key={item.id} className={item.id === selectedId ? "bg-muted/40" : undefined}>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4"><StatusBadge status={item.status} />{item.errorCode ? <span className="mt-1 block text-xs text-destructive">{item.errorCode}</span> : null}</td>
                  <td className="px-4 py-4">第 {item.generation} 代</td>
                  <td className="px-4 py-4">{item.attemptCount} 次</td>
                  <td className="px-4 py-4">{formatTime(item.updatedAt)}</td>
                  <td className="px-4 py-4">
        ...
        <aside className="border-t bg-muted/15 p-5 xl:border-l xl:border-t-0" aria-label="发送尝试详情">
          ...
          {!delivery ? <div className="mt-8 text-center text-sm text-muted-foreground">尚未选择投递</div> : attempts.isPending ? <div className="mt-8 text-sm text-muted-foreground">正在读取发送尝试…</div> : attempts.isError ? (
            <div className="mt-6 rounded-lg border border-destructive/30 p-4 text-sm" role="alert">
              <p>尝试记录读取失败，不影响投递列表。</p>
        ...
        <DialogContent>
          ...
          {hasDuplicateRisk ? (
            <label className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950">
              <input className="mt-1" type="checkbox" checked={duplicateConfirmed} onChange={(event) => setDuplicateConfirmed(event.target.checked)} />
              我确认该投递可能已经送达，再次发送可能产生重复通知。系统会创建新的投递代数，不修改原记录。
            </label>
          ) : null}
    ...
    <>
      <section className="grid gap-4 lg:grid-cols-2" aria-label="通知渠道">
        {query.data.map((channel) => {
          ...
          return (
            <article key={channel.channel} className="rounded-lg border bg-card p-5">
...
function PolicyCard({ scope, gateway, onUnauthorized }: { scope: PolicyScope; gateway: NotificationGateway; onUnauthorized: () => void }) {
  ...
  if (query.isError) return <div className="rounded-lg border border-destructive/30 bg-card p-5" role="alert"><h2 className="font-semibold">{policyLabels[scope]}</h2><p className="mt-2 text-sm">该策略暂时无法读取，其他策略不受影响。</p><Button className="mt-4" size="sm" variant="outline" onClick={() => void query.refetch()}><RefreshCw />重新加载</Button></div>
  if (query.isPending || !policy) return <div className="min-h-64 rounded-lg border bg-card p-5"><p className="text-sm text-muted-foreground">正在读取{policyLabels[scope]}策略…</p></div>

  const canUpdate = hasAction(policy.allowedActions, "UPDATE")
  return (
    <article className="rounded-lg border bg-card p-5">
      <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">{policyLabels[scope]}</h2><p className="mt-1 text-xs text-muted-foreground">版本 v{policy.version}</p></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={policy.enabled} disabled={!canUpdate || mutation.isPending} onChange={(event) => setDraft({ ...policy, enabled: event.target.checked })} />启用</label></div>
      {scope === "system-alerts" ? (
...
function ChannelChoices({ label, value, disabled, onChange }: { label: string; value: DeliveryChannel[]; disabled: boolean; onChange: (value: DeliveryChannel[]) => void }) {
  return (
    <fieldset><legend className="text-sm font-medium">{label}</legend><div className="mt-2 flex gap-4">{(["WECOM", "EMAIL"] as const).map((channel) => <label key={channel} className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={disabled} checked={value.includes(channel)} onChange={(event) => onChange(event.target.checked ? [...value, channel] : value.filter((item) => item !== channel))} />{channelLabels[channel]}</label>)}</div></fieldset>
  )
}
...
function TemplatesView({ gateway, onUnauthorized }: { gateway: NotificationGateway; onUnauthorized: () => void }) {
  ...
  return (
    <>
      <section className="overflow-hidden rounded-lg border bg-card" aria-label="通知模板版本">
        {[...grouped.entries()].map(([type, templates]) => (
          <div key={type} className="grid gap-3 border-b p-4 last:border-b-0 lg:grid-cols-[16rem_1fr]">
            ...
            <div className="space-y-2">
              {templates.map((template) => (
                <div key={template.version} className="flex flex-wrap items-center justify-between gap-3 rounded-md border bg-background px-3 py-2 text-sm">
      ...
      <Dialog open={operation !== null} onOpenChange={(open) => { if (!open) close() }}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          {operation?.kind === "preview" ? <><label className="grid gap-2 text-sm font-medium">模板变量（JSON）<textarea className="min-h-32 rounded-md border bg-background px-3 py-2 font-mono text-sm" value={variables} onChange={(event) => setVariables(event.target.value)} /></label>{preview ? <pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded-md bg-muted p-4 text-sm">{preview}</pre> : null}</> : <label className="grid gap-2 text-sm font-medium">操作原因<Input value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} /></label>}
          {mutation.isError ? <MutationError error={mutation.error} /> : null}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${danger ? "bg-red-100 text-red-800" : success ? "bg-emerald-100 text-emerald-800" : warning ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-700"}`}>{statusLabels[status] ?? (status === "ENABLED" ? "已启用" : status === "DISABLED" ? "已停用" : status)}</span>
}

function MutationError({ error }: { error: unknown }) {
  return <div className="flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm" role="alert"><AlertTriangle className="mt-0.5 size-4 shrink-0" /><div><p>{error instanceof Error ? error.message : "操作未完成，请检查后重试。"}</p>{error instanceof ApiError ? <p className="mt-1 text-xs text-muted-foreground">错误码：{error.code}</p> : null}</div></div>
}
